...
import time
from ..items import App, AffinityAppMediator, AppHighlight, AppHighlightMediator
import scrapy
import re
import uuid
import hashlib
...
class AppDataScrapeSpider(scrapy.spiders.SitemapSpider):
    ...
    def parse_app_page(self, response):
        ...
        app_full_description_list = response.xpath(
            "(//div[@class='block vc-app-listing-about-section__description'])[1]").getall()
        ...
        #     print("description_sentence", description_sentence)
        #     if description_sentence != "\n":
        #         app_full_description = app_full_description + \
        #             (str(description_sentence).strip().strip() + '\n')

        for description_sentence in app_full_description_list:
            if len(description_sentence) > 0:
                app_full_description = app_full_description + description_sentence
        ...
        app_highlights_list_obtained = response.xpath(
            "//div[@class='block vc-verified-attributes-section__block']//ul[@class='vc-verified-attributes-section__list']//p[@class='vc-verified-attributes-section__item-text']//text()").getall()

        app_highlights_list = []
        for a in app_highlights_list_obtained:
            if len(a) > 1:
                app_highlights_list.append(a.strip())
        ...
        for app_highlight in app_highlights_list:
            yield AppHighlight(
                highlight_text=app_highlight
            )
            yield AppHighlightMediator(
                app_id=app_id,
                highlight_text=app_highlight
            )
