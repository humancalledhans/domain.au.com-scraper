...
def crawl(settings={}, spider_name="domain_spider", spider_kwargs={}):
    ...
    try:
        spider_key = urlparse(spider_kwargs.get("sitemap_urls")[0]).hostname if spider_kwargs.get(
            "sitemap_urls") else urlparse(spider_cls.start_urls[0]).hostname
    except Exception:
    ...
    if is_in_aws():
        ...
        feed_uri = "file://{}/%(name)s-{}-%(time)s.json"
